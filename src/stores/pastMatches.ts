...
import type { Match } from '../services/PastMatches/types'
import type { APIResponse } from '../services/types'
import { API } from '../services'
import { AxiosError } from 'axios'

interface GetPastMatchesParams {
  league_id?: number
  videogame?: number
}
...
export const usePastMatchesStore = defineStore('pastMatchesStore', () => {
  ...
  async function dispatchGetPastMatches(
    params: GetPastMatchesParams = {},
    ...
    try {
      const { status, data } = await API.pastMatches.getPastMatches(params)
      if (status === 200) {
